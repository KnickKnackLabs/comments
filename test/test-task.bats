...
@test "options-only calls use the configured default test directory" {
  run comments test --jobs 1 --filter '^standard comments surfaces exist$'

  [ "$status" -eq 0 ]
  [[ "$output" == *'1..1'* ]]
  [[ "$output" == *'ok 1 standard comments surfaces exist'* ]]
}
...
@test "an explicit test target takes precedence over the configured default" {
  ...
  run comments test --jobs 1 "$target"
...
@test "relative test targets resolve from the repository root" {
  run comments test --jobs 1 test/skeleton.bats --filter '^standard comments surfaces exist$'
...
@test "whitespace-bearing explicit test targets remain one argument" {
  ...
  run comments test --jobs 2 "$target"
...
@test "public Comments test path runs separate BATS files concurrently" {
...
@test "public Comments test path runs tests within one BATS file concurrently" {
